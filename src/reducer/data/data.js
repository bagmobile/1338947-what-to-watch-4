    ...
    case ActionType.UPDATE_FAVORITE_STATUS_MOVIE:
      const movies = state.movies.map((movie) =>
        movie.id === action.payload.id
          ? extend(movie, {isFavorite: action.payload.isFavorite})
          : movie
      );
      const promoMovie = state.promoMovie.id === action.payload.id
        ? extend(state.promoMovie, {isFavorite: action.payload.isFavorite})
        : state.promoMovie;
